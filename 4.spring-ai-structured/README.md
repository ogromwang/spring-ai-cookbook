...
大语言模型（LLM）的核心是 **Transformer 架构**，它本质上是一个**概率生成器**：

```
输入文本 → Tokenization → Embedding → Transformer → Softmax → Token 概率分布
```

**生成过程**：
1. 模型根据输入，预测下一个 token 的概率分布
2. 使用采样策略（如 greedy、top-k、temperature）从分布中选择 token
3. 将选中的 token 加入输出，重复直到 EOS

**关键点**：
- 每次生成都是**概率采样**，理论上任何 token 都有可能
- 模型无法区分"有效 token"和"无效 token"
- 例如：生成 JSON 时，可能随时采样到语法错误的 token（如引号不配对）
...
| 问题 | 原因 |
|-----|------|
| JSON 引号缺失 | 随机采样可能跳过引号 token |
| 解释文字混入 | 模型倾向于"人性化"回复 |
| 语法错误 | 模型不懂 JSON 语法，只懂语言模式 |
| 格式不一致 | 今天返回 JSON，明天返回 Markdown |

### 解决方案：约束采样 / 约束解码

**核心思想**：将模型约束在根据提供的模式有效的 token，而不是所有可用的 token。
...
├─────────────────────────────────────────────────────────────┤
│  原始流程:                                                   │
│  输入 → 模型 → 词汇表 V → 采样 → 下一个 token               │
│                                                              │
│  约束流程:                                                   │
│  输入 → 模型 → 词汇表 V → 过滤无效 token → 约束采样 → 下一个 token│
│                    ↑                                        │
│                    |                                        │
│           根据 JSON Schema / Pydantic 过滤
│           只保留语法正确的 token
└─────────────────────────────────────────────────────────────┘
```
...
1. **提示阶段引导**：在提示词末尾追加格式指令（如 JSON Schema）
2. **模型调用**：LLM 收到带格式要求的完整提示
3. **响应阶段约束**：使用 `StructuredOutputConverter` 强制约束输出格式
4. **自动解析**：将返回的结构化文本反序列化为 Java 对象
...
- [大模型结构化输出技术解析 - DataLearner](https://www.datalearner.com/blog/1051723018180775)
